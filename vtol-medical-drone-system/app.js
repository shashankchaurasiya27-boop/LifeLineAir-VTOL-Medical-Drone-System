...
function initializeDashboard() {
  ...
  if (successRateEl) {
    ...
    setTimeout(() => {
      successRateEl.textContent = appData.kpi_data.success_rate + '%';
    }, 1600);
  }
  if (avgDeliveryEl) {
    animateValue(avgDeliveryEl, 0, appData.kpi_data.avg_delivery_time, 1800);
    setTimeout(() => {
      avgDeliveryEl.textContent = appData.kpi_data.avg_delivery_time + ' min';
    }, 1900);
  }
  ...
  updateIntervals.dashboard = setInterval(() => {
    updateDashboardData();
  }, 5000);
}
...
function createInventoryCard(supply) {
  ...
  const stockPercentage = (supply.stock / (supply.min_threshold * 2)) * 100;
  const statusClass = supply.status.toLowerCase();
  const fillClass = supply.status === 'Good' ? 'good' : 'low';
  
  card.innerHTML = `
    <div class="inventory-header">
      <div class="inventory-item">${supply.item}</div>
      <div class="inventory-status inventory-status--${statusClass}">${supply.status}</div>
    </div>
    <div class="stock-info">
      <div class="stock-numbers">
        <span class="current-stock">${supply.stock} units</span>
        <span class="min-threshold">Min: ${supply.min_threshold}</span>
      </div>
      <div class="stock-bar">
